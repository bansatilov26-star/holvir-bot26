...
async def create_yookassa_payment(amount, description, username):  
    ...
    payload = {  
        ...
        "confirmation": {  
            "type": "redirect",  
            "return_url": "https://t.me/holvir_assistant_bot",  
        },  
        "description": f"Holvir Start — {description}",  
        ...
        async with aiohttp.ClientSession() as session:  
            async with session.post(url, json=payload, headers=headers, auth=auth) as resp:  
                if resp.status == 200:  
                    ...
                    logging.error(f"Ошибка ЮKassa: {resp.status} — {error_text}")  
                    return None, None  
    except Exception as e:  
        logging.error(f"Ошибка запроса к ЮKassa: {e}")  
        return None, None  
...
def payment_result_keyboard():  
    ...
    keyboard.add(  
        InlineKeyboardButton("📢 Перейти в Telegram-канал", url=TG_CHANNEL)  
    )  
    return keyboard  
...
@dp.message_handler(commands=["develop"], state="*")  
async def develop_command(message: types.Message, state: FSMContext):  
    await state.finish()  
    await message.answer(  
...
@dp.message_handler(lambda message: message.text == "🚀 Начать проект", state="*")  
...
@dp.message_handler(lambda message: message.text == "➡️ Продолжить", state=None)  
...
async def process_q2(message: types.Message, state: FSMContext):  
    if message.text == "✍️ Другое":  
        await ProjectForm.other_text.set()  
        keyboard = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)  
        keyboard.add(KeyboardButton("🔙 Главное меню"))  
        await message.answer("✍️ Напишите свой вариант:", reply_markup=keyboard)  
        return  
    async with state.proxy() as data:  
...
@dp.message_handler(state=ProjectForm.other_text)  
async def process_other_text(message: types.Message, state: FSMContext):  
    async with state.proxy() as data:  
        if "q2" not in data:  
            data["q2"] = message.text  
            await ProjectForm.question3.set()  
...
async def process_username(message: types.Message, state: FSMContext):  
    ...
    payment_id, payment_url = await create_yookassa_payment(  
        amount=price,  
        description=f"Holvir Start — {username}",  
        username=username,  
    )  
    ...
    if payment_url:  
        ...
        await message.answer(  
            ...
            f"Пожалуйста, напишите нам: {SUPPORT_USERNAME}",  
            reply_markup=main_keyboard(),  
        )  
...
async def process_payment(callback_query: types.CallbackQuery):  
    ...
    if status == "succeeded":  
        ...
        await callback_query.message.answer(  
            ...
            "📸 Пожалуйста, напишите менеджеру и отправьте ему скриншот оплаты.\n"  
            "Важно, чтобы на скриншоте был виден код платежа — так мы сможем быстрее начать с вами работать.\n\n"  
            f"👨‍💻 Менеджер: {SUPPORT_USERNAME}\n\n"  
            "После проверки мы сразу приступим к работе над вашим проектом.",  
            reply_markup=payment_result_keyboard(),  
        )  
    elif status == "pending":  
        await callback_query.message.answer(  
            "⏳ Платёж пока не подтверждён.\n\n"  
            "Пожалуйста, подождите немного и нажмите «✅ Я оплатил» снова.",  
            reply_markup=main_keyboard(),  
        )  
    else:  
        await callback_query.message.answer(  
            "❌ Платёж не найден или отменён.\n\n"  
            "Если вы уверены, что оплатили — напишите менеджеру:\n"  
            f"👨‍💻 {SUPPORT_USERNAME}",  
            reply_markup=main_keyboard(),  
        )  
